...
#ifdef ODIRECT
        if(!psync) {
		fd = open(test_file.c_str(), O_RDWR | O_CREAT | O_DIRECT,
				S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
        }
        else {
		fd = open(test_file.c_str(), O_RDWR | O_DIRECT,
				S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
        }
#else
        if(!psync) {
...
begin:
	...
	*/
	...
	if(!(io_bench::get_test_type(argv[1]) == SEQ_WRITE_READ && cur_test == SEQ_WRITE) && !psync) {
		time_stats_print(&main_stats, (char *)"--------------- stats");
		printf("%s throughput: %3.3f MB\n", io_bench::get_test_str(cur_test).c_str(),
			((float)n_threads * (float)((file_size_bytes) >> 20))
			/ (float) time_stats_get_avg(&main_stats));
	}

	if(io_bench::get_test_type(argv[1]) == SEQ_WRITE_READ && cur_test == SEQ_WRITE) {
		cur_test = SEQ_READ;
		goto begin;
	}
