...
int rpc_lease_change(int mid, int rid, uint32_t inum, int type, uint32_t version, addr_t blknr, \
				     int sync, enum lease_qualifier lq, gid_t chown_target_gid, int chino_parent_ino)
{
	uint64_t start_tsc;
	...
	snprintf(msg->data, RPC_MSG_BYTES, "|lease |%u|%u|%d|%u|%lu|%u|%u|%d", rid, inum, type, version, blknr, lq, chown_target_gid, chino_parent_ino);
	mlfs_printf("\x1b[33m [L] trigger lease acquire: inum[%u] type[%d] version[%u] blknr[%lu] qualifier[%u] chown_target_gid[%d] chino_parent_ino[%d] (%s) \x1b[0m\n",
			inum, type, version, blknr, lq, chown_target_gid, chino_parent_ino, sync?"SYNC":"ASYNC");
