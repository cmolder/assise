...
static int get_secondary_groups(uid_t uid, gid_t **buf) {
	...
	if (!buf) {
		mlfs_debug("[DEBUG] Could not allocate groups array\n");
		return -1;
	} 
	...
	pw = getpw(uid, pwbuf);
	if (!pw) {
		mlfs_debug("[DEBUG] Could not get passwd struct\n");
		return -1;
	}

	if (getgrouplist(pw->pw_name, pw->pw_gid, buf, NGROUPS) == -1) {
		mlfs_debug("[DEBUG] Could not get group list\n");
		return -1;
	}
...
static int should_group_bits_apply(uid_t uid, gid_t primary_gid, gid_t inode_gid) {
	...
	if (get_secondary_groups(uid, &secondary_grp_list) != 0) {
